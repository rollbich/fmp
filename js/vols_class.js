...
class stats_period {
	...
	async init() {
		...
		if (year_fin == this.year) {
			...
			this.show_result_vols("accueil_vols");
			this.show_result_regs("accueil_reguls");
		} else {
	...
	async show_result_vols(containerId) {
		...
		<h2>Dates équivalentes Year-1: ${reverse_date(this.period.lastyear.start_day)} au ${reverse_date(this.period.lastyear.end_day)}</h2>
		<h2>Dates équivalentes 2019: ${reverse_date(this.period[2019].start_day)} au ${reverse_date(this.period[2019].end_day)}</h2><br>
