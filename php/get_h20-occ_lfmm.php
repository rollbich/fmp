...
function get_courbe($type, $zone, $year, $month, $day) {
	//$login = 'olafatco_tds_lfmm-bf';
	//$password = '(dgac2021)';
	$url = "https://lfmm-fmp.fr/b2b/json/$year/$month/$year$month$day-$type-$zone.json";
	$ch = curl_init();
...
try {
	if (isset($_GET["type"]) && isset($_GET["zone"]) && isset($_GET["date"])) {
		...
		if ($type !== "H20" && $type != "Occ") $ok_type = false;
		if ($zone !== "est" && $zone !== "west") $ok_zone = false;
		$d = explode("-", $date);
		$year = $d[0];
		$month = $d[1];
		$day = $d[2];
		...
		$resultat = get_courbe($type, $zone, $year, $month, $day);
